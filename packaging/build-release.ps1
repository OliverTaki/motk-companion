...
function Copy-RelativeFile([string]$RelativePath) {
  ...
  $target = Join-Path (Join-Path $stage 'app') $RelativePath
...
New-Item -ItemType Directory -Force -Path (Join-Path $stage 'app'),(Join-Path $stage 'runtime'),(Join-Path $stage 'scripts') | Out-Null
...
Copy-Item -LiteralPath (Join-Path $runtimeExtract 'node.exe') -Destination (Join-Path $stage 'runtime\node.exe')
Copy-Item -LiteralPath (Join-Path $runtimeExtract 'LICENSE') -Destination (Join-Path $stage 'runtime\LICENSE-node.txt')
Copy-Item -LiteralPath (Join-Path $runtimeExtract 'README.md') -Destination (Join-Path $stage 'runtime\README-node.md')
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'windows\install.ps1') -Destination (Join-Path $stage 'install.ps1')
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'windows\update.ps1') -Destination (Join-Path $stage 'update.ps1')
...
$releaseMetadata | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $stage 'RELEASE.json') -Encoding utf8
...
$sbom | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $stage 'SBOM.spdx.json') -Encoding utf8

Get-ChildItem -LiteralPath $stage -File -Recurse | ForEach-Object { $_.LastWriteTimeUtc = $buildTime.UtcDateTime }
$manifestFiles = Get-ChildItem -LiteralPath $stage -File -Recurse | Sort-Object { $_.FullName.Substring($stage.Length).Replace('\', '/') } | ForEach-Object {
  [ordered]@{
...
[ordered]@{ format = 1; release = $releaseName; files = @($manifestFiles) } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $stage 'manifest.json') -Encoding utf8
(Get-Item -LiteralPath (Join-Path $stage 'manifest.json')).LastWriteTimeUtc = $buildTime.UtcDateTime
